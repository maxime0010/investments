...
def calculate_price_target_statistics(cursor, closing_price_dict):
    query = f"""
        SELECT 
            ...
            AVG(CASE WHEN r.date >= DATE_SUB(NOW(), INTERVAL {DAYS_RECENT} DAY) AND a.overall_success_rate > {SUCCESS_RATE_THRESHOLD} THEN (r.adjusted_pt_current - {closing_price_dict.get('ticker', 'NULL')}) / {closing_price_dict.get('ticker', 'NULL')} * 100 ELSE 0 END) AS expected_return_combined_criteria
        FROM (
            SELECT 
                ticker,
                analyst_name,

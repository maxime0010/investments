...
def update_existing_portfolio_simulation_super_bullish(cursor, today, closing_prices):
    ...
    cursor.execute("SELECT MAX(date) FROM portfolio_simulation_super_bullish WHERE date < %s", (today,))
    ...
    cursor.execute("SELECT ticker, quantity, total_value FROM portfolio_simulation_super_bullish WHERE date = %s", (latest_portfolio_date,))
    ...
    for ticker, quantity, total_value in existing_portfolio:
        ...
        cursor.execute("""
            UPDATE portfolio_simulation_super_bullish
            SET date_sell = %s, 
                stock_price_sell = %s, 
                total_value_sell = %s, 
                evolution = %s
...
def insert_new_portfolio_simulation_super_bullish(cursor, today, new_portfolio, closing_prices):
    """Insert the new portfolio simulation for the current date."""
    closing_price_dict = {ticker: Decimal(price) for ticker, price in closing_prices}

    # Get the total value from selling the previous portfolio
    cursor.execute("SELECT SUM(total_value_sell) FROM portfolio_simulation_super_bullish WHERE date_sell = %s", (today,))
    ...
    for ranking, (ticker, expected_return, _) in enumerate(new_portfolio, start=1):
        ...
        cursor.execute("""
            INSERT INTO portfolio_simulation_super_bullish (date, ranking, ticker, stock_price, quantity, total_value)
            VALUES (%s, %s, %s, %s, %s, %s)
...
try:
    ...
    if datetime.today().weekday() == 6:
        ...
        # Step 1: Update the most recent existing portfolio in portfolio_simulation_super_bullish
        update_existing_portfolio_simulation_super_bullish(cursor, today, closing_prices)

        # Step 2: Fetch the new portfolio based on analysis_simulation
        new_portfolio = fetch_new_portfolio(cursor)
        if new_portfolio:
            # Step 3: Insert the new portfolio into portfolio_simulation_super_bullish
            insert_new_portfolio_simulation_super_bullish(cursor, today, new_portfolio, closing_prices)

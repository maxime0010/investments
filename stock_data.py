...
def insert_stock_data(stock_data):
    try:
        ...
        add_stock = ("INSERT INTO stock (ticker, name, indices, website) "
                     "VALUES (%(ticker)s, %(name)s, %(indices)s, %(website)s) "
                     "ON DUPLICATE KEY UPDATE name = VALUES(name), indices = VALUES(indices), website = VALUES(website)")
...
# Script execution
try:
    create_stock_table()  # Ensure the stock table exists
    fetch_and_store_stock_data(sp500_tickers)
    exit_program()
except Exception as e:

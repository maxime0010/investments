...
def update_existing_portfolio_simulation_bullish(cursor, today, closing_prices):
...
def insert_new_portfolio_simulation_bullish(cursor, today, new_portfolio, closing_prices):
    """Insert the new portfolio simulation for the current date."""
    closing_price_dict = {ticker: Decimal(price) for ticker, price in closing_prices}

    # Get the total value from selling the previous portfolio
    cursor.execute("SELECT SUM(total_value_sell) FROM portfolio_simulation_bullish WHERE date_sell = %s", (today,))
...
try:
    ...
    if datetime.today().weekday() == 6:
        ...
        # Step 1: Update the most recent existing portfolio in portfolio_simulation_bullish
        update_existing_portfolio_simulation_bullish(cursor, today, closing_prices)

        # Step 2: Fetch the new portfolio based on analysis_simulation
        new_portfolio = fetch_new_portfolio(cursor)
        if new_portfolio:
            # Step 3: Insert the new portfolio into portfolio_simulation_bullish
            insert_new_portfolio_simulation_bullish(cursor, today, new_portfolio, closing_prices)

...
def update_existing_portfolio(cursor, today, closing_prices):
    """Update the most recent existing portfolio with the latest closing prices."""
...
def insert_new_portfolio(cursor, today, new_portfolio, closing_prices):
    """Insert the new portfolio for the current date."""
    closing_price_dict = {ticker: Decimal(price) for ticker, price in closing_prices}

    # Get the total value from selling previous portfolio
...
try:
    ...
    # Step 2: Fetch the new portfolio based on analysis
    new_portfolio = fetch_new_portfolio(cursor)
    if new_portfolio:
        # Step 3: Insert the new portfolio
